...
class WorkingTimes extends Component {
  ...
  async componentDidMount() {
    ...
    const employees = await axios.get("http://localhost:8080/api/employees/", config);
    this.setState({
      ...this.state,
  ...
  render() {
    ...
    const setEmployee = async (e) => {
      ...
      const availabilities = await axios.get("http://localhost:8080/api/availability/employee/"+e.target.value, config);
      this.setState({
